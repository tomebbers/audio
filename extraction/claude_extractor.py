...
def extract_audiogram(image_bytes: bytes, mime_type: str) -> AudiogramData:
    ...
    return _build_audiogram_data(raw_data)
...
def _build_audiogram_data(raw: dict) -> AudiogramData:
...
def _build_ear_thresholds(side: str, raw: dict) -> EarThresholds:
    """Build EarThresholds from raw extracted data."""
...
def _parse_threshold_dict(raw: dict) -> dict[int, float | None]:
    """Parse a frequency -> threshold dict, converting string keys to int."""
    result = {}
    if not raw:
        return result
...
def _build_speech_data(side: str, raw: dict) -> SpeechData:
    """Build SpeechData from raw extracted data."""
    if not raw:
...
def _validate_audiogram_data(data: AudiogramData) -> None:
    """Validate extracted data for clinical plausibility. Add notes for issues."""
    for ear in [data.right_ear, data.left_ear]:
        ...
        # Check BC not worse than AC (with tolerance)
